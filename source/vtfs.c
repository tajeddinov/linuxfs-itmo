...
ssize_t vtfs_read(struct file *filp, char __user

*buffer,
size_t length, loff_t
*offset);

ssize_t vtfs_write(struct file *filp, const char __user
...
memset(file_info
->content.data, 0, file_info->content.allocated);
}
}
...
memcpy(file_info
->content.data + *offset, tmp_buffer, length);
kfree(tmp_buffer);
...
void vtfs_kill_sb(struct super_block *sb) {
    ...
    printk(KERN_INFO
    "vtfs super block is destroyed. Unmount successfully.\n");
}
